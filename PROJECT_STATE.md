...
| **Last updated** | 2026-08-18 (PD-099 CLOSED — all seven dimensions approved) |
...
- [x] **PD-099 CLOSED — Dimension 7 (Conflicting Allowances) APPROVED as RESOLVED BY
      EXISTING RULES** (2026-08-18). **All seven dimensions are now decided.** For the
      **three** grant-carried allowance types the specification confirms — **quality,
      concurrency, devices** (§13.2, *"[CONFIRMED, values OPEN]"*) — the general rule the
      dimension asked for **already exists and is MAX**. **Content remains UNION and is not
      an allowance-resolution rule.** **Territory and effective dates decide which grants
      participate first.** **Independent rights, contractual constraints, asset/channel
      policy and service availability remain authoritative** under the existing eleven
      checks, with §6.7's already-decided **most restrictive** rule governing the comparison
      **between** the commercial value and every independent constraint. **No new universal
      MIN, SUM, priority, winner or most-restrictive-across-commercial-grants rule is
      introduced, and no approved dimension is reopened.** **FUTURE ALLOWANCE SCOPE — a
      completeness constraint, not a resolution rule: no fourth grant-carried allowance type
      exists and none is created here; should one ever be approved it must carry its own
      explicit resolution rule before implementation and does not inherit MAX.**
      Fully-approved decisions **5 → 6**. Recorded in `DECISIONS.md` §2,
      `PRODUCT_SPEC.md` §12.1/§13.4, `REQUIREMENTS.md` FR-PKG-08, and §7 **D-039** below.
      **No new decision ID. PD-013, PD-049 Q2, PD-056, PD-057, PD-038 and PD-040 unchanged.**
- [x] **PD-099 Territory sub-decision APPROVED** (2026-08-18): when a subscriber is
      physically in a **served** territory, **only commercial grants territorially eligible
      for the subscriber's CURRENT territory participate** in effective entitlement
      resolution. An ineligible grant participates in **none** of content UNION, quality
      MAX, concurrency MAX or device MAX. **Q-T2 is resolved by the same predicate — no
      separate allowance rule was created.** **The grant is NOT cancelled, the subscription
      is NOT terminated, the grant remains held**, and it participates again on return to a
      covered territory, **subject to its effective dates**. Evaluated in **grant-set
      construction**, alongside temporal filtering and **before** the eleven checks —
      **no twelfth check**. **Current territory only; the home territory is never
      substituted** (`FR-TRV-04`, P0). **No roaming limit, travel duration,
      percentage-of-time rule or country list created.** Content rights (checks 6, 9) and
      service availability (check 11) remain **independently authoritative**;
      `NOT_IN_PACKAGE` remains the commercial reason code; the **Case B no-workaround
      discipline is unchanged**. **Dimensions approved: 1, 2, 3, 4, 5 and 6 of 7; one
      remains OPEN — conflicting allowances.** Recorded in `DECISIONS.md` §2,
      `PRODUCT_SPEC.md` §12.1/§12.3/§13.4, `REQUIREMENTS.md` FR-PKG-08, `USER_FLOWS.md`
      UF-26, and §7 **D-038** below. **No new decision ID.**
- [x] **PD-099 Content Entitlement sub-decision APPROVED** (2026-08-16): the effective
      **commercial** content entitlement is the **UNION** of the content included by the
      participating grants, and **a second participating grant can never reduce the
      catalogue**. **Commercial reach only** — content rights, territory rights, service
      availability, distribution-mode rights and asset/channel policy all remain
      independently authoritative, and **commercial entitlement never creates or expands a
      content right**. The three-stage pipeline is unchanged and **no twelfth check** is
      added. It **complements the approved Quality rule without changing it**, supplying the
      *reach* predicate that rule presupposed. **Modes remain a rights matter; no
      mode-scoped commercial grant is introduced.** **Dimensions approved: 1, 2, 3, 4 and 6
      of 7; two remain OPEN — territory and conflicting allowances.** Recorded in
      `DECISIONS.md` §2, `PRODUCT_SPEC.md` §12.1/§13.4, `REQUIREMENTS.md` FR-PKG-08, and §7
      **D-037** below. **Approved with it:** the audit consequence — **D-030 amended** to
      require the evidence to identify the **contributing grant set**, with **K-010**
      updated to record that the requirement is settled while its representation is not.
      **No new decision ID.**
- [x] **PD-099 Devices sub-decision APPROVED** (2026-08-16): **D-1 + D-2.** **D-1** — the
      effective device allowance is **`MAX(applicable grant device allowances)`**;
      **allowances are NOT summed** (2, 5 and 3 give **5**). It stays a **grant-level
      commercial input enforced at account level**, server-side, independent of device class,
      content rights, concurrency, territory rights and device capability. **D-2** — a
      reduction below the registered count **grandfathers existing devices**: no automatic
      de-registration, no automatic selection policy, no playback-blocking rule; only **new**
      registration is denied, while `registered_count >= effective_device_limit`. **D-2
      required no change to the authorization model** — enforcement has always been
      prospective at registration, and check 7 never consults the count. **The eleven checks
      are unchanged; no twelfth. No device value inferred — PD-038 remains OPEN.**
      **Dimensions approved: 1, 2, 3 and 6 of 7; three remain OPEN.** Recorded in
      `DECISIONS.md` §2, `PRODUCT_SPEC.md` §6.4/§13.4, `REQUIREMENTS.md`
      FR-DEV-04/FR-PKG-08, `USER_FLOWS.md` UF-20A, and §7 **D-036** below. **No new decision
      ID.** Territory applicability of *"applicable"* remains governed by PD-099 Territory
      (OPEN) and is recorded as an unresolved dependency, not resolved.
- [x] **PD-099 Effective Dates sub-decision APPROVED** (2026-08-16): **one inseparable block
      of three rules.** **(1) Half-open interval `[start, end)`** — `effective_from` inclusive,
      `effective_until` exclusive; grants `[10:00, 12:00)` and `[12:00, 15:00)` do not overlap.
      **(2) Future grants do not participate** before their start — no entitlement, no quality,
      no concurrency. **(3) Dual boundary enforcement** — scheduled processing **and** an
      authoritative per-authorization re-evaluation; **the client never determines whether a
      grant is active**. UTC and server-time authority unchanged; no local-device time
      semantics. Rules 1 and 2 **supply the temporal membership predicate** the approved
      Quality and Concurrency rules presuppose — **neither of those rules is changed**.
      **Dimensions approved: 1, 2 and 6 of 7; four remain OPEN.** Recorded in `DECISIONS.md`
      §2, `PRODUCT_SPEC.md` §12.1/§13.4, `REQUIREMENTS.md` FR-PKG-03/FR-PKG-08, and §7
      **D-035** below. **Mid-session grant expiry expressly NOT decided (PD-056 OPEN); no
      twelfth check; no new decision ID.** Audit requirements recorded as **K-011**, not
      solved.
- [x] **PD-099 Quality sub-decision APPROVED** (2026-08-16): **Q1 + Q2 as an inseparable
      pair.** **Q1** — the effective commercial quality ceiling is the **MAXIMUM** among the
      grants that reach the requested asset. **Q2** — **only active grants that actually
      entitle that asset participate**, so a grant that does not include it may not constrain
      its quality. Q1 alone would permit **entitlement leakage across products**, which is why
      the halves are recorded together. **§6.7 is expressly not the basis** — it is
      concurrency-scoped; the rule rests on positive grant semantics, §12.1 **check 10**, and
      **PD-049 Q1**. Second-stage constraints stay authoritative, and **commercial entitlement
      never creates or expands a content right**. **Dimensions approved: 1 and 2 of 7; five
      remain OPEN.** Recorded in `DECISIONS.md` §2, `PRODUCT_SPEC.md` §12.1/§13.4,
      `REQUIREMENTS.md` FR-PKG-08, and §7 **D-034** below. **No quality value set, no twelfth
      check, no new decision ID.** An authorization-output/evidence gap is recorded as
      **K-010**, not solved.
- [x] **PD-099 Territory — Case B no-workaround UX discipline APPROVED** (2026-08-16):
      a **UX constraint only**, recorded under **dimension 5**, which **remains OPEN**. The
      experience for a territory-caused denial **must not** suggest returning home, changing
      location, circumvention or a VPN, and **must not** imply the subscription expired, the
      account was cancelled, or the grant was deleted; it **may** state the content is not
      included in the package **in the current territory** and **may** show a commercial
      action **only where a qualifying product exists there**. **The reason code remains
      `NOT_IN_PACKAGE` — no new reason code, no new authorization check.** The commercial
      action's shape is **UNDECIDED** (PD-049 Q2, PD-013, both OPEN). Recorded in
      `DECISIONS.md` §2, `PRODUCT_SPEC.md` §12.3, `USER_FLOWS.md` UF-07 F2 / UF-13 F2 /
      UF-18 / UF-26 F3, and §7 **D-033** below. **PD-004, PD-095, PD-099 Concurrency,
      PD-049 Q2, PD-013 and PD-007 are all unchanged, and no add-on decision is implied.**
...
| K-011 | Info | **IDENTIFIED AUDIT REQUIREMENTS — recorded, not solved** (2026-08-16, arising from the PD-099 Effective Dates approval). To reconstruct *"why was Grant X active at time T?"* the evidence must eventually support: (1) a **reproducible evaluation timestamp**; (2) an **auditable effective interval**; (3) **grant state/transition history** sufficient to reconstruct active/inactive status at any past instant. **A concrete gap underlies (3):** `FR-SUB-04` (**P0**) makes **subscription** history append-only, but **no equivalent requirement exists for grants** — so if a grant's effective dates can be edited in place, past decisions become unreproducible, defeating §26.3's retention of playback decisions as rights-compliance evidence. **D-030 was deliberately not modified and no decision ID was created.** To be resolved before Phase 14/15. |
| K-010 | Info | **IDENTIFIED GAP — partially closed 2026-08-16.** **The contributing-grant-set REQUIREMENT is now decided** — D-030 as amended with D-037 requires the evidence to be capable of identifying the contributing grant set. **Its REPRESENTATION, and the other three values, remain an open gap.** Original entry follows. **IDENTIFIED GAP — recorded, not solved** (2026-08-16, arising from the PD-099 Quality approval). The specification does not yet define an authorization **output or evidence representation** for: (1) participating grants · (2) commercial effective quality · (3) the binding independent quality constraint · (4) final effective playback quality. **An underlying gap makes this unavoidable:** `PRODUCT_SPEC.md` §12.4 does not list effective quality among playback-session properties, so **no quality output is specified at all** — there is nothing for the player to honour and nothing to record. **D-030 was deliberately not modified and no decision ID was created.** To be resolved before Phase 14/15. |
...
| D-039 | 2026-08-18 | **PD-099 Dimension 7 (Conflicting Allowances) APPROVED — RESOLVED BY EXISTING RULES. PD-099 is CLOSED, all seven dimensions decided.** For the three grant-carried allowance types the specification confirms, the resolution rules already exist: **quality = MAX**, **concurrency = MAX**, **devices = MAX**. **Content remains UNION** and is not an allowance-resolution rule. **Territory and effective dates determine which grants participate** before any dimension-specific rule is evaluated. **Independent rights, contractual constraints, asset/channel policy and service availability remain authoritative**, evaluated independently under the existing eleven checks. **No new universal MIN, SUM, priority, winner or "most restrictive across commercial grants" rule is introduced. No approved dimension is reopened. No twelfth check.** **FUTURE ALLOWANCE SCOPE:** **no fourth grant-carried allowance type exists and none is created**; should PD-013 or a later approved decision ever introduce one, **it must carry its own explicit resolution rule before implementation and does not inherit MAX**. | **Accepted — product owner decision** | Recorded as a **sub-decision under the existing PD-099 identifier** — no new decision ID. Dimension 7 was written as the **catch-all** for a set of dimensions of which, at the time, *"none … is decided here"*; the specific rules now cover the space exhaustively, leaving no fourth kind of disagreement for a general rule to settle. **The allowance inventory is closed at three** — §13.2 enumerates package feature entitlements as *"quality ceiling, concurrency, device allowance"*, marked **`[CONFIRMED, values OPEN]`**, so the list is settled and only the values are open; profile limits are **per account** (§5.3), catch-up windows are *"per channel and per rights agreement"* (§17), and distribution modes are a **rights** matter (§15.3, L-002) — none is grant-carried, so none can conflict between grants. A universal most-restrictive rule was rejected as a **regression**: it would contradict dimensions 1–3 (allowances of 2, 1 and 4 would yield 1 rather than the approved 4) and defeat PD-049 Q1; restriction belongs **between** layers, per §6.7, never **within** the commercial layer. Priority was rejected because it needs **a total order over grants the specification does not supply** — the same ground on which priority was rejected for content reach. The future-allowance constraint exists because MAX is demonstrably right for the three confirmed allowances, each **positive and totally ordered**, and is not self-evidently right for a boolean, unordered, or less-is-better allowance; declaring *"MAX forever"* would assert a rule for allowance types that do not exist, which `CLAUDE.md` §1 prohibits. **Audit unchanged:** D-030 as amended already requires the contributing grant set, which covers `MAX(X, Y)` directly; **K-010 and K-011 remain the recorded non-blocking gaps and were not modified.** |
| D-038 | 2026-08-18 | **PD-099 Territory sub-decision APPROVED — current-territory grant participation.** When a subscriber is physically in a **served** territory, **only commercial grants territorially eligible for the subscriber's CURRENT territory participate** in effective entitlement resolution. An ineligible grant participates in **none** of content UNION, quality MAX, concurrency MAX or device MAX. **The grant is NOT cancelled, the subscription is NOT terminated, the grant remains held**, and it participates again on return to a covered territory, **subject to its effective dates**. **Q-T2 is resolved by the same predicate; no separate allowance rule is created. No twelfth check. Dimension 5 of 7; PD-099 overall remains OPEN.** | **Accepted — product owner decision** | Recorded as a **sub-decision under the existing PD-099 identifier** — no new decision ID. The rule occupies a slot the specification had already cut: §13.4 read *"Which grants participate is set at stage 1 and, for territory, remains [OPEN — PD-099 Territory]"*, and the approved Devices sub-decision recorded that whether *"applicable"* carries **grant territorial** applicability was governed by this dimension — so supplying one predicate resolves both Q-T1 and Q-T2 and leaves all four approved arithmetics textually unchanged. Quality needs no reinterpretation (Q2 admits only *"active grants that actually entitle the requested asset"*, and an excluded grant never reaches that test); the union rule is scoped to *"participating"* grants, so a smaller catalogue in an uncovered territory is not the reduction that rule forbids; and **D-2 absorbs a territorial reduction unamended and reversibly**, because the over-limit condition is **emergent, not stored**. Basis: PD-004's binding consequence that packages *"MAY differ by territory — so each must be modelled as territory-scoped, not global"*, §13.5's *"territory attaches to grant eligibility"*, §13.2's `[PROPOSED]` package eligibility, and the approved Case B discipline, whose trigger presupposes exactly this state. **PD-095 is not reopened** — it settled the territory input for checks 6, 9 and 11 while listing *"Package entitlement"* among the six travelling conditions without saying which territory it reads; this fills that gap. **PD-099 Conflicting Allowances, PD-056, PD-049 Q2, PD-013 and PD-057 are unchanged.** |
| D-037 | 2026-08-16 | **PD-099 Content Entitlement sub-decision APPROVED — UNION of the participating grants.** The effective **commercial** content entitlement is the union of the content included by the participating grants, and **a second participating grant can never reduce the catalogue**. **Commercial reach only:** it may not override or weaken content rights, territory rights, service availability, distribution-mode rights, asset/channel policy, or any of the eleven checks, and **commercial entitlement may never create or expand a content right**. **No twelfth check. Dimension 4 of 7; PD-099 overall remains OPEN.** | **Accepted — product owner decision** | Recorded as a **sub-decision under the existing PD-099 identifier** — no new decision ID. Union rather than intersection or priority because the model already implies it: grants are **positive** (§13.1 *"may access"*, check 4 *"includes"*) and **no grant-level denial mechanism exists anywhere in the specification**. Intersection would defeat PD-049 Q1 and visibly shrink the entitlement-filtered surfaces required by `FR-HOME-01`, `FR-VOD-02` and `FR-SCH-01` (all P0); priority would require a total order the specification does not supply. It **complements the approved Quality rule without changing it** — quality resolves MAX among *"grants that reach the requested asset"*, and union is what defines *reach*, a predicate that rule already presupposed. Modes stay a rights matter (`FR-CUP-03`, `FR-RST-02`, `FR-RGT-03`, L-002) and **no mode-scoped commercial grant is introduced**. Territory continues to filter set membership at stage 1 and remains **OPEN**. |
| D-036 | 2026-08-16 | **PD-099 Devices sub-decision APPROVED — D-1 + D-2.** **D-1:** the effective device allowance is **`MAX(applicable grant device allowances)`** — **allowances are NOT summed** (2, 5, 3 → **5**, not 10 and not 2); a **grant-level commercial input enforced at account level**, server-side, independent of device class, content rights, concurrency, territory rights and device capability. **D-2:** a reduction below the registered count **grandfathers existing devices** — no automatic de-registration, no automatic selection policy, no playback-blocking rule — and denies **new** registration while `registered_count >= effective_device_limit`. **Dimension 3 of 7; PD-099 overall remains OPEN. No device value inferred — PD-038 unchanged.** | **Accepted — product owner decision** | Recorded as a **sub-decision under the existing PD-099 identifier** — no new decision ID. **D-2 needed no change to the authorization model**, and that is the key compatibility finding: `FR-DEV-04` (P0) never located enforcement in time, §6.4 and UF-20A both place it at **registration**, and **check 7** is a per-device status test that never consults the count — so an over-limit device still passes it, which *is* the grandfathering D-2 requires. MAX matches the concurrency direction, but the two resources differ: a concurrent stream is a **runtime** slot that churns, a registered device is **persistent** — which is exactly why concurrency needed no reduction rule and devices did. Consistent with PD-095, which likewise holds that travel never strips what the subscriber already has. **Unresolved dependency recorded, not resolved:** whether *"applicable"* also carries **grant territorial** applicability is governed by **PD-099 Territory** (OPEN), a different concept from the *content-rights* territory named in D-1's independence list. |
| D-035 | 2026-08-16 | **PD-099 Effective Dates sub-decision APPROVED — one inseparable block of three rules.** **(1)** Grants use a **half-open** effective interval **`[start, end)`** — `effective_from` inclusive, `effective_until` exclusive — so adjacent grants never overlap at a shared instant. **(2)** A grant whose start is in the future **does not participate** in entitlement, quality or concurrency before that instant. **(3)** Boundaries are enforced by **both** scheduled processing **and** an authoritative **per-authorization re-evaluation**; the client never determines whether a grant is active. UTC and server-time authority unchanged. **Dimension 6 of 7; PD-099 overall remains OPEN.** | **Accepted — product owner decision** | Recorded as a **sub-decision under the existing PD-099 identifier** — no new decision ID. Rule 3 closes a gap the review identified: **`FR-PKG-03` (P0) demanded *"immediately"* while specifying no mechanism at all**, whereas the rights analogue mandates two (`FR-RGT-06`, L-003 — *"a single mechanism can fail silently"*). Cache invalidation elsewhere is event-driven, and a boundary crossing is not an event, so without Rule 3 the P0 obligation had no enforcement path. Rules 1 and 2 supply the **temporal membership predicate** the already-approved Quality (*"active"*) and Concurrency (*"applicable"*) rules presuppose without defining — **neither of those rules is changed**, and half-open removes the non-determinism both had at a shared boundary, which `FR-PKG-08` (P0) requires them not to have. **Mid-session grant expiry is expressly NOT decided and must not be inferred from "immediately"; PD-056 remains OPEN.** **No twelfth check** — effective-date filtering is grant-set construction before the existing checks. |
| D-034 | 2026-08-16 | **PD-099 Quality sub-decision APPROVED — Q1 + Q2 as an inseparable pair.** **Q1:** where several applicable grants reach the requested asset, the effective commercial quality ceiling is the **MAXIMUM** among them. **Q2:** **only active grants that actually entitle the requested asset participate** — a grant that does not include it MUST NOT constrain its quality. Second-stage constraints remain authoritative and conjunctive, and **commercial entitlement never creates or expands a content right**. **Dimension 1 of 7; PD-099 overall remains OPEN.** | **Accepted — product owner decision** | Recorded as a **sub-decision under the existing PD-099 identifier** — no new decision ID. The pair is inseparable because **Q1 alone permits entitlement leakage across products**: a 4K sports grant would raise a movie to 4K although it grants no movie entitlement. **§6.7 is expressly NOT the basis** — it is concurrency-scoped and never mentions quality; the rule rests on positive grant semantics (§13.1), §12.1 check 10's per-asset quality permissions, and PD-049 Q1. The domain asymmetry with the approved concurrency MAX is deliberate and correct: concurrency is an **account-level** pool, quality is **asset-scoped** — same function, different resources. **No quality value is set, no twelfth check is added, and PD-013, PD-017, PD-049 Q2, PD-099 Territory, PD-099 Effective Dates, PD-099 Devices and PD-099 Conflicting Allowances are all unchanged.** |
| D-033 | 2026-08-16 | **PD-099 Territory — Case B no-workaround UX discipline APPROVED.** Where a held commercial grant does not participate because the subscriber is in a territory it does not cover, the experience **MUST NOT** suggest returning home, changing location, circumvention or a VPN, and **MUST NOT** imply the subscription expired, the account was cancelled, or the grant was deleted. It **MAY** state the content is not included in the package **in the current territory**, and **MAY** show a commercial action **only where a qualifying product exists there**. **Reason code remains `NOT_IN_PACKAGE`; no new code, no new check. Dimension 5 (territory eligibility) remains OPEN.** | **Accepted — product owner decision** | Recorded as a **constraint under dimension 5**, not a resolution of it, and as a **sub-decision under the existing PD-099 identifier** — no new decision ID. It needed stating rather than assuming: the no-workaround discipline already attaches to `TERRITORY_RESTRICTED` and `SERVICE_NOT_AVAILABLE` because both are non-commercial denials, but **`NOT_IN_PACKAGE` is a commercial code that normally *does* offer a path** (§12.3), so it does not inherit the discipline by default. The constraint is also **independent of how dimension 5 resolves** — the Case B situation is reachable under both candidate readings. The commercial action's shape stays **UNDECIDED** pending PD-049 Q2 and PD-013. |
...
| D-030 | 2026-08-16 · **AMENDED 2026-08-16** | **Where several grants could each reach the same asset, the playback-authorization evidence must record which grant the decision rested on.** **AMENDMENT (product owner, with D-037):** where several grants **contribute** to the effective entitlement, the evidence **MUST be capable of identifying the CONTRIBUTING GRANT SET**, rather than requiring a single *"winning grant"*. The original wording is retained above and is **broadened, not superseded**. | **Accepted — consequence of D-028** | `CLAUDE.md` §12 requires authorization decisions to be logged "with enough detail to prove compliance to a rights holder". Under a single-grant model the grant was implicit; under `0..N` it is not. *"The account was entitled"* is not evidence a rights holder can audit. **No legal-log entry was created for this**: it makes no claim about law or about what any contract permits, and the rights-side rule it supports is already recorded as L-002. |
