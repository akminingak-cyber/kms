...
**Status:** DRAFT — awaiting product approval · **PD-004, PD-007, PD-008, PD-092, PD-095, PD-099 APPROVED** · **PD-049 Q1 APPROVED (structural)**
**Version:** 1.12
**Date:** 2026-08-18 (rev. 1.12 — PD-099 closed, dimension 7 resolved by existing rules: FR-PKG-08 updated)
...
| FR-PKG-03 | Package expiration makes entitlement ineffective immediately, enforced by **both** scheduled boundary processing **and** an authoritative per-authorization re-evaluation *(PD-099 effective-dates sub-decision APPROVED)*. **This governs the entitlement effect only; behaviour for an in-progress session is [OPEN — PD-056].** | **P0** |
...
| FR-PKG-08 | **Where several grants coexist, entitlement resolution is deterministic and server-side.** **Concurrency is resolved as an account-level pool: the effective commercial allowance is the MAXIMUM applicable grant allowance, not the sum** *(PD-099 concurrency sub-decision APPROVED)*, and independent constraints — including rights-agreement concurrency limits — remain authoritative. **Quality is resolved per asset: the effective commercial ceiling is the MAXIMUM among grants that reach the requested asset, and only those grants participate** *(PD-099 quality sub-decision APPROVED — Q1 and Q2 are an inseparable pair)*, with independent constraints — content rights, the rights-agreement ceiling and the asset playback policy — remaining authoritative. **Grants participate only while `effective_from ≤ now < effective_until` — a half-open interval evaluated against authoritative server time** *(PD-099 effective-dates sub-decision APPROVED)*, applied as grant-set construction before the authorization checks. **Device allowance is resolved as the MAXIMUM applicable grant allowance, not the sum, enforced at account level** *(PD-099 devices sub-decision APPROVED)*. **Commercial content reach is the UNION of the content included by the participating grants; a second participating grant never reduces the catalogue** *(PD-099 content-entitlement sub-decision APPROVED)* — commercial reach only, with content rights, territory, service availability and distribution-mode rights remaining independently authoritative. **In a served territory, only grants territorially eligible for the subscriber's CURRENT territory participate; an ineligible grant contributes no content, no quality, no concurrency and no device allowance, is NOT cancelled, remains held, and participates again on return subject to its effective dates** *(PD-099 territory sub-decision APPROVED)* — evaluated in grant-set construction, never against the home territory, and adding no authorization check. **Conflicting allowances require no further rule: each confirmed grant-carried allowance already resolves by MAX, and the most restrictive applicable constraint governs between the commercial value and every independent constraint. No universal MIN, SUM, priority or winner rule exists** *(PD-099 conflicting-allowances sub-decision APPROVED — resolved by existing rules; PD-099 CLOSED)*. **No fourth grant-carried allowance type exists; should one ever be approved it must carry its own explicit resolution rule before implementation and does not inherit MAX.** | **P0** |
