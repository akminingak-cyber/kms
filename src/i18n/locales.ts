...
export const LOCALE_META: Record<
  Locale,
  { native: string; short: string; htmlLang: string; ogLocale: string; shareCard: string }
> = {
  // `shareCard` is the image every social platform renders when this locale's
  // pages are pasted into a feed. Both files are generated by
  // `scripts/og-image.mjs` from the hero headline, so they cannot drift from the
  // page they represent. A single shared card would show English copy to the
  // Georgian audience the site is written for.
  ka: {
    native: 'ქართული',
    short: 'KA',
    htmlLang: 'ka-GE',
    ogLocale: 'ka_GE',
    shareCard: '/og-image-ka.png',
  },
  // Open Graph wants a full language_TERRITORY tag; a bare "en" is silently
  // ignored by Facebook's parser.
  en: { native: 'English', short: 'EN', htmlLang: 'en', ogLocale: 'en_US', shareCard: '/og-image.png' },
};
